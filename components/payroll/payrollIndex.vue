<template>
  <!-- ESTE ARCHIVO FLUJE COMO INDEX PARA LA NAVEGACION -->
  <div class="flex flex-col md:flex-col gap-6 p-4">
    ...
    <nav class="bg-white rounded-2xl shadow-sm p-4">
      <div class="flex justify-between">
        <div class="tabs-navigation flex justify-between p-2">
          <div v-for="(tab, index) in tabs" :key="index">
            <button
              :class="['px-4 py-2 rounded-lg transition-all', tab.selected ? 'text-transparent bg-clip-text bg-gradient-to-r from-primario to-secundario group-hover:from-secundario group-hover:to-primario' : 'text-gray-500']"
    ...
    <div class="dynamic-content">
      <SalaryBreakdown v-if="currentTab === 'SalaryBreakdown'" />
      <TaxDefinition v-if="currentTab === 'TaxDefinitions'" />
      <Payslips v-if="currentTab === 'Payslips'" />
      <Payroll v-if="currentTab === 'Payroll'" />
    </div>
...
import SalaryBreakdown from '@/components/payroll/SSalaryBreakdown.vue'
import TaxDefinition from '@/components/payroll/STaxDefinition.vue'
import Payslips from '@/components/payroll/SPayslips.vue'
import Payroll from '@/components/payroll/SPayrollDetails.vue'
...
export default defineComponent({
  ...
  data () {
    return {
      chartHeight: '60rem',
      currentTab: 'SalaryBreakdown',
      selected: false,
      tabs: [
        { id: 'SalaryBreakdown', label: 'Salary Breakdown', selected: true },
        { id: 'TaxDefinitions', label: 'TaxDefinitions', selected: false },
        { id: 'Payslips', label: 'Payslips', selected: false },
        { id: 'Payroll', label: 'Payroll', selected: false }
      ],
      cards: [
        {
          value: '5,205,350.00',
